function ADMM_price_based_m!(Case_name::String, results::Dict,ADMM::Dict, model_e::Model, model_r::Model, model_ccb::Model, dict::Dict, ETS_data::Dict, LINEAR::Bool) #, conditional_case::Bool)
    ...
    for iter in iterations
        if convergence == 0
            ...
            for t in  model_e.ext[:sets][:T]
                ...
                # NO Learning
                # Cum_abat[t] = model_e.ext[:parameters][:Cum_abat][t] = model_e.ext[:parameters][:Cum_abat][t-1]
                ...
                # NO Learning
                # Cum_remov[t] = model_r.ext[:parameters][:Cum_remov][t] = model_r.ext[:parameters][:Cum_remov][t-1]
            ...
            model_r.ext[:parameters][:ec_bar] = Dict(t => results["ec"][iter,t+1] for t in model_r.ext[:sets][:T] )
            model_r.ext[:parameters][:x_bar] = Dict(t => results["x"][iter,t+1] for t in model_e.ext[:sets][:T] )
            ...
            model_e.ext[:parameters][:rc_e_bar]  = Dict(t => results["rc"][iter,t+1] for t in model_r.ext[:sets][:T] )
            ...
            # # Imbalances
            ADMM["Imbalances"]["EAC"][iter,:] = results["x"][iter,:]  + results["rc"][iter,:] - results["ec"][iter,:] 
            ...
            ADMM["Imbalances"]["CDR"][iter,:] = results["x"][iter,:]  + results["rc"][iter,:] - results["ec"][iter,:]
            # # Primal residuals
    ...
    # NO Learning
    # Cum_remov_extract = Cum_NoLearning_remov
    # Cum_abat_extract =Cum_NoLearning_abat
    ...
    Total_budget = sum((results["ec"][ADMM["n_iter"],:].*[values(EAC_price[t]) for t in 0:1:T_OPT] -results["rc"][ADMM["n_iter"],:].*[values(CDR_price[t]) for t in 0:1:T_OPT])) 
    ...
    ETS_avg = sum([EAC_price[t].*results["e"][ADMM["n_iter"],t+1] for t in 0:1:T_OPT])./sum([results["e"][ADMM["n_iter"],t+1] for t in 0:1:T_OPT])

    Tot_EmW =  sum(1/(1+DRc)^t*((BETA*(EM_ref)^(GAMMA+1))/(GAMMA+1))  for t in model_r.ext[:sets][:T]) - Total_costs_abat
    Tot_EmS = Tot_EmW - sum([1/(1+DRc)^t*EAC_price[t]*results["ec"][ADMM["n_iter"],t+1] for t in 0:1:T_OPT])
    Tot_ReS =  sum([1/(1+DRc)^t*CDR_price[t]*results["rc"][ADMM["n_iter"],t+1] for t in 0:1:T_OPT]) - Total_costs_rem
    Tot_GoS = sum([1/(1+DRc)^t*EAC_price[t]*results["ec"][ADMM["n_iter"],t+1] for t in 0:1:T_OPT]) - sum([1/(1+DRc)^t*CDR_price[t]*results["rc_r"][ADMM["n_iter"],t+1] for t in 0:1:T_OPT]) 

    Tot_ES = Tot_EmS + Tot_ReS + Tot_GoS
    ...
    push!(dict[(Case_name,"Learning_gains_per_rem")], (Total_costs_rem_NL -Total_costs_rem)/Total_removals)
    ...
    push!(dict[(Case_name,"Emitter_cost_avg")], ETS_avg + Total_costs_abat/Total_emissions)
...
function Emitter!(model::Model)
    ...
    ec = model.ext[:variables][:ec] = @variable(model, ec[T], lower_bound=0, base_name="Emission allowances bought in year t (equal to the gross emissions)")
    x = model.ext[:variables][:x] = @variable(model, x[T], base_name="Net emissions")

    # for t in T
    #     fix(x[t],  Timeseries_data[convert.(Int64,t+1), "ETS_cap [EUA/y]"], force=true)
    # end 
    ...
    Credits_e =  model.ext[:constraints][:Credits_e] = @constraint(model, [t=T],
    (ec[t]  >= x[t]))
    Budget_e =  model.ext[:constraints][:Budget_e] = @constraint(model, [t=[T_OPT]],
    (sum(x[i] for  i in 0:t)  == BUDGET))
    ...
    ( ρ./2) * sum( DiscountFactor[t]*((ec[t] - (ec_bar[t]  + 1/N*(x[t] + rc_e_bar[t] -  ec[t])))^2) for t in T) .* a  )
...
function Update_emitter!(model::Model)
   ...
   ec = model.ext[:variables][:ec] = @variable(model, [t=T], lower_bound=0, base_name="Emission allowances bought in year t (equal to the gross emissions)")
   x = model.ext[:variables][:x] = @variable(model, [t=T], base_name="Net emissions")
    ...
    for t in T
        ...
        delete(model,model.ext[:constraints][:Credits_e][t])
    end
    delete(model,model.ext[:constraints][:Budget_e][T_OPT])
...
function Remover!(model::Model)
    ...
    ( ρ./2) * sum( DiscountFactor[t]*((rc[t] - (rc_bar[t] - 1/N*(x_bar[t] + rc[t] -  ec_bar[t])))^2) for t in T) .* a )
...
function CentralBank!(model::Model)
    ...
    x_bar = model.ext[:parameters][:x_bar]
    ec_bar = model.ext[:parameters][:ec_bar]        # ADMM penalty term
    rc_bar = model.ext[:parameters][:rc_bar]        # ADMM penalty term
    rc_e_bar = model.ext[:parameters][:rc_e_bar]    # ADMM penalty term
    rc_r_bar = model.ext[:parameters][:rc_r_bar]    # ADMM penalty term
    ...
    # Penalty term ETS market
    ( ρ1./2) * sum( DiscountFactor[t]*((rc_e[t] - (rc_e_bar[t] - 1/N*(x_bar[t] + rc_e[t] -  ec_bar[t])))^2) for t in T) .* a   +
...
function Joint_optimum_C0!(REMOVALS::Bool, BUDGET)
    ...
    Cap_con =  model.ext[:constraints][:Cap_con] = @constraint(model, [t=[T_OPT]],
    (sum(ec[i] - rc[i] for i in 0:t) == BUDGET ))
    ...
    model.ext[:objective] = @objective(model, Min, 
    # MACC
    sum((DiscountFactor[t])*(A_a*(1-e[t]./EM_REF) + B_a*((1-e[t]./EM_REF)).^(2)/2  + (A_r*(r[t]/EM_REF) + B_r*(r[t]/EM_REF).^(2)/2)) for t in T)) # NOTE: the /EM_REF is checked and correct
...
function plot_model!(Case_name::String, C_nr, model_e, model_r, model_ccb, ADMM, results, dict::Dict, plotting::Bool)
    ...
    if !plotting
        ...
        ymax = 650.0
        pp = stackedarea(Timeseries_data[1:1:(T_OPT+1),"Year"], [EM -REM],   ylims = (-200, 3000),  yticks =0:500:3000, labels = ["Emissions"  "Removals"], foreground_color_legend = nothing, title = "", ylabel = L"Emissions or Removals [MtCO$_2$/yr]", xlabel = "Years", legend = (0.02, 0.90), linecolor = Color, color = Color, alpha = Alpha, right_margin = 5mm)    
        ...
        plot!(twiny(pp), Timeseries_data[1:1:(T_OPT+1),"Year"], [Timeseries_data[1:1:(T_OPT+1), "ETS_cap [EUA/y]"]./1], lw=1.5, ylims = (-200,3000), label = ["ETS cap"],foreground_color_legend = nothing, xformatter=_->"", yformatter=_->"", xlabel = "", ylabel="", legend = (0.02,0.98), color = ["black"  "navy" "black" "brown"], ls = [:dash :dash :dash :dash], margin = 0mm)
        ...
        ppp = groupedbar(Timeseries_data[1:1:(T_OPT+1),"Year"], [EM -REM], bar_position =:stack, xticks = (2025:5:(2024+T_OPT)), ylims = (-1100, 3100), yticks =-1000:250:3100, labels = ["Emissions"  "Removals"], foreground_color_legend = nothing, title = "", ylabel = L"Emissions or Removals [MtCO$_2$/yr]", xlabel = "Years", legend = (0.02, 0.85), lw = 0.75, color = Color, linecolor = [:gray31 :gray31], alpha = Alpha, right_margin = 5mm)
        plot!(twiny(ppp), Timeseries_data[1:1:(T_OPT+1),"Year"], [Timeseries_data[1:1:(T_OPT+1), "ETS_cap [EUA/y]"]./1], lw=[1 1], xticks = (2025:5:(2024+T_OPT)), ylims = (-1100, 3100), label = ["ETS cap"],foreground_color_legend = nothing, xformatter=_->"", yformatter=_->"", xlabel = "", ylabel="", legend = (0.02,0.98), color = ["black"  "black" "black" "brown"], ls = [:dash :dash :dash :dash], margin = 0mm)
        plot!(twiny(ppp), Timeseries_data[1:1:(T_OPT+1),"Year"], [EM-REM], lw=0, xticks = (2025:5:(2024+T_OPT)), ylims = (-1100, 3100), label = ["Net emissions"],foreground_color_legend = nothing, xformatter=_->"", yformatter=_->"", xlabel = "", ylabel="", legend = (0.02,0.91), color = ["black"  "black" "black" "brown"], ls = [:dash :dash :dash :dash], margin = 0mm, markershape = :xcross, markersize = 2)
