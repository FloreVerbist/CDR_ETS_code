function ADMM_price_based_m!(Case_name::String, results::Dict,ADMM::Dict, model_e::Model, model_r::Model, model_ccb::Model, dict::Dict, ETS_data::Dict, LINEAR::Bool) #, conditional_case::Bool)
    ...
    for iter in iterations
        if convergence == 0
            ...
            # # Imbalances
            ADMM["Imbalances"]["EAC"][iter,:] = [model_e.ext[:parameters][:CAP][t] for t in  model_e.ext[:sets][:T]]  + results["rc_e"][iter,:] - results["ec"][iter,:] 
    ...
    results["r"][ADMM["n_iter"],:] = results["r"][ADMM["n_iter"],:]
    ...
    # include("case_processing.jl")
    global model_ccbr = Model(optimizer_with_attributes(Gurobi.Optimizer,"OutputFlag" =>  0))
    # global model_r = Model(optimizer_with_attributes(Gurobi.Optimizer,"OutputFlag" =>  0))
    iterations2 = ProgressBar(1:ADMM_max_it-1)

    process_sets_parameters!(model_ccbr, Parameters,Timeseries_data, T_OPT)
    # process_sets_parameters!(model_r, Parameters,Timeseries_data, T_OPT)
    Cum_remov = model_ccbr.ext[:parameters][:Cum_remov] 
    model_ccbr.ext[:parameters][:CDR_price] = Dict(t => results["CDR_price"][ADMM["n_iter"],t+1] for t in model_ccbr.ext[:sets][:T]) #FV

    convergence2 = 0 
    global results2 = Dict()
    global ADMM2 = Dict()
    CentralBank_recalibrate!(model_ccbr)
    # Remover_recalibrate!(model_r)
    define_results!(Parameters::DataFrame,results2,ADMM2)

    for iter2 in iterations2
        if convergence2 == 0
            # Calculate penalty terms ADMM and update price to most recent value

            # predefine required credits released on ETS market
            model_ccbr.ext[:parameters][:rc_e_bar] = Dict(t => results["rc_e"][ ADMM["n_iter"],t+1] for t in model_e.ext[:sets][:T])
            # model_r.ext[:parameters][:rc_e_bar] = Dict(t => results["rc_e"][ ADMM["n_iter"],t+1] for t in model_e.ext[:sets][:T])

            if iter2 > 1
                # model_r.ext[:parameters][:CDR_price] = Dict(t => results2["CDR_price"][iter2-1,t+1] for t in model_r.ext[:sets][:T])
                model_ccbr.ext[:parameters][:CDR_price] = Dict(t => results2["CDR_price"][iter2-1,t+1] for t in model_ccbr.ext[:sets][:T]) #FV

                model_ccbr.ext[:parameters][:rc_r_bar] = Dict(t => results2["rc_r"][iter2-1,t+1] for t in model_ccbr.ext[:sets][:T])
                # model_r.ext[:parameters][:rc_bar] = Dict(t => results2["rc"][iter2-1,t+1] for t in model_r.ext[:sets][:T])

                # model_r.ext[:parameters][:ρ] = 0.01
                model_ccbr.ext[:parameters][:ρ1] =0.5
                model_ccbr.ext[:parameters][:ρ2] = 0.5
            ...
            for t in  model_ccbr.ext[:sets][:T]
                ...
                D_r = model_ccbr.ext[:parameters][:D_r]  
                C_r = model_ccbr.ext[:parameters][:C_r]  
                M_r = model_ccbr.ext[:parameters][:M_r] 
                K_r = model_ccbr.ext[:parameters][:K_r] 
                U_r = model_ccbr.ext[:parameters][:U_r] 
                ...
                remov = collect(0:st:rem_max)
                CDR_price_nom = maximum([0,model_ccbr.ext[:parameters][:CDR_price][t]]) # FV: maximum maybe not correct here?
                LAMDA_CDR = @.(CDR_price_nom)
                _, j = findmin(abs.(RC(remov,t, Cum_remov).-LAMDA_CDR))
                Remov_t = maximum([0, remov[j]]).*EM_REF

                model_ccbr.ext[:parameters][:r][t] = maximum([0,Remov_t])
                
                Cum_remov[t] = model_ccbr.ext[:parameters][:Cum_remov][t] = model_ccbr.ext[:parameters][:Cum_remov][t-1] + model_ccbr.ext[:parameters][:r][t]


                results2["Cum_remov"][iter2, t+1] = Cum_remov[t]

            end

            results2["r"][iter2,:] = [model_ccbr.ext[:parameters][:r][t] for t in model_ccbr.ext[:sets][:T]]
            
                        
            # Update_remover_recalibrate!(model_r)
            # variables_extract(model_r)
        
            # results2["rc"][iter2,:] = [value.(model_r.ext[:variables][:rc][t]) for t in model_r.ext[:sets][:T]]
            model_ccbr.ext[:parameters][:rc_bar]  = Dict(t => results2["r"][iter2,t+1]  for t in model_ccbr.ext[:sets][:T] )
            

            #Solve updated problem
            Update_CentralBank_recalibrate!(model_ccbr)
            variables_extract(model_ccbr)
            results2["rc_r"][iter2,:] = [value.(model_ccbr.ext[:variables][:rc_r][t]) for t in model_ccbr.ext[:sets][:T]]
            # model_r.ext[:parameters][:rc_r_bar] = Dict(t => results2["rc_r"][iter2,t+1] for t in model_r.ext[:sets][:T] )
            ...
            # # Imbalances
            ADMM2["Imbalances"]["CDR"][iter2,:] = [value.(model_ccbr.ext[:parameters][:r][t]) - value.(model_ccbr.ext[:variables][:rc_r][t]) for t in model_ccbr.ext[:sets][:T]]

            # # Primal residuals

            ADMM2["Residuals"]["Primal_CDR"][iter2] = sqrt(sum(ADMM2["Imbalances"]["CDR"][iter2,:].^2))

            if iter2 > 1 
                ADMM2["Residuals"]["Dual_RC"][iter2] = sqrt(sum(( (results2["rc"][iter2,:] - results2["rc"][iter2-1,:])).^2))
                ADMM2["Residuals"]["Dual_RC_R"][iter2] = sqrt(sum(( (results2["rc_r"][iter2,:] - results2["rc_r"][iter2-1,:])).^2))
            else
            end
        
            # Price updates 
            results2["CDR_price"][iter2+1,:] = results2["CDR_price"][iter2,:] - model_ccbr.ext[:parameters][:ρ] .*ADMM2["Imbalances"]["CDR"][iter2,:] 

            # Progress bar
            set_description(iterations2, string(@sprintf("Primal CDR %.3f --  Dual RC %.3f -- Dual RC R %.3f",  ADMM2["Residuals"]["Primal_CDR"][iter2],  ADMM2["Residuals"]["Dual_RC"][iter2], ADMM2["Residuals"]["Dual_RC_R"][iter2])))

            # Check convergence: primal and dual satisfy tolerance OR primal remains high and dual near zero. 
            # In case the latter happens, the solution needs to be inspected to ensure this can be interpreted as an equilibrium.
            if (ADMM2["Residuals"]["Primal_CDR"][iter2] <= model_e.ext[:parameters][:epsilon] && ADMM2["Residuals"]["Dual_RC"][iter2] <= model_e.ext[:parameters][:epsilon] &&   ADMM2["Residuals"]["Dual_RC_R"][iter2] <= model_e.ext[:parameters][:epsilon])
                convergence2 = 1
            end
            ADMM2["n_iter"] = copy(iter2)
        end
    end
    results2["r"][ADMM["n_iter"],:] = results2["r"][ADMM["n_iter"],:].*FRAC_R  # actually should be below those cummulative

    Cum_remov_extract = Dict( i =>  results2["Cum_remov"][ADMM2["n_iter"],i+1] for i in 0:T_OPT)
    Cum_remov_extract[-1] = Cum_remov_init # creation of element in case that t = 0
    ...
    for t in 0:T_OPT
        if value.(model_ccbr.ext[:parameters][:r][t]) == 0.0 
            # model_ccbr.ext[:parameters][:CDR_price][t] = convert(Vector{Float64}, RC(0.0,t, Cum_remov))
            results2["CDR_price"][ADMM2["n_iter"],t+1] = RC(0.0, t,  Cum_remov_extract)[1]
        else
        end
    end
    ...
    parameters_extract(model_ccbr)
    Total_costs_rem = sum(1/(1+DRc)^t*quadgk(var -> fr(var, t, Cum_remov_extract), 0, r[t]./EM_REF)[1].*EM_REF for t in model_ccbr.ext[:sets][:T])[1]
    Total_costs_rem_NL = sum(1/(1+DRc)^t*quadgk(var -> fr(var, t, Cum_NoLearning_remov), 0, r[t]./EM_REF)[1].*EM_REF for t in model_ccbr.ext[:sets][:T])[1]
    parameters_extract(model_e)
    Total_costs_abat = sum(1/(1+DRc)^t*quadgk(var -> fa(var, t, Cum_abat_extract), 0,(EM_REF-e[t])./EM_REF)[1].*EM_REF for t in model_ccbr.ext[:sets][:T])[1]
    Total_costs_abat_NL = sum(1/(1+DRc)^t*quadgk(var -> fa(var, t, Cum_NoLearning_abat), 0,(EM_REF-e[t])./EM_REF)[1].*EM_REF for t in model_ccbr.ext[:sets][:T])[1]
    EAC_price = model_e.ext[:parameters][:EAC_price]
    CDR_price = model_ccbr.ext[:parameters][:CDR_price] # FV: maybe this one should be changed as well. 


    Total_costs = Total_costs_rem + Total_costs_abat
    
   
    Total_budget = sum((results["ec"][ADMM["n_iter"],:].*[values(EAC_price[t]) for t in 0:1:T_OPT] -results2["rc"][ADMM2["n_iter"],:].*[values(CDR_price[t]) for t in 0:1:T_OPT])) 
    Total_net_emissions = sum(results["e"][ADMM["n_iter"],:] - results2["r"][ADMM2["n_iter"],:])
    Total_emissions  = sum(results["e"][ADMM["n_iter"],:]) 
    Total_removals = sum(results2["r"][ADMM2["n_iter"],:]) 
    Total_abated =  (T_OPT+1)*EM_REF - Total_emissions
    ETS_max = maximum([EAC_price[t] for t in 0:1:T_OPT])
    RC_max = maximum([results2["CDR_price"][ADMM2["n_iter"],t+1] for t in 0:1:T_OPT])
...
function Emitter!(model::Model)
    ...
    ( ρ./2) * sum( DiscountFactor[t]*((ec[t] - (ec_bar[t] + 1/N*(CAP[t] + rc_e_bar[t] -  ec[t])))^2) for t in T) .* a  )
...
function Remover_recalibrate!(model::Model)
    model.ext[:variables] = Dict()
    model.ext[:constraints] = Dict()
    ...
    Banking_r =  model.ext[:constraints][:Banking_r] = @constraint(model, [t=T],
    (sum(rc[i] for  i in 0:t)  <= sum(r[i]   for i in 0:t)))
    # Banking_ccb_end =  model.ext[:constraints][:Banking_ccb_end] = @constraint(model, [T_OPT],
    # (sum(rc_e_bar[i] for  i in 0:T_OPT)  - sum(rc[i]   for i in 0:T_OPT)) == 0)
...
function Update_remover_recalibrate!(model::Model)
    model.ext[:variables] = Dict()
    ...
    # delete(model, model.ext[:constraints][:Banking_ccb_end][T_OPT])
...
function CentralBank!(model::Model)
    ...
    Banking_ccb_end =  model.ext[:constraints][:Banking_ccb_end] = @constraint(model, [T_OPT],
    (sum(rc_e[i] for  i in 0:T_OPT)  - sum(rc_r[i]   for i in 0:T_OPT)) == 0)

    model.ext[:objective] = @objective(model, Min, 
    # MACC
    (- sum(DiscountFactor[t]*rc_e[t].*((EAC_price[t]-P_CAP)) for t in T)) +  #sum(DiscountFactor[t]*(rc_r[t].*(CDR_price[t])) for t in T)  +
    # Penalty term ETS market
...
function CentralBank_recalibrate!(model::Model)
    ...
    # Constraints 
    Banking_ccb =  model.ext[:constraints][:Banking_ccb] = @constraint(model, [t=T],
    (sum(rc_e_bar[i] for  i in 0:t)  <= sum(rc_r[i]   for i in 0:t)))
    Banking_ccb_end =  model.ext[:constraints][:Banking_ccb_end] = @constraint(model, [T_OPT],
    (sum(rc_e_bar[i] for  i in 0:T_OPT)  - sum(rc_r[i]   for i in 0:T_OPT)) == 0)

    model.ext[:objective] = @objective(model, Min, 
    # MACC
    (sum(DiscountFactor[t]*rc_r[t].*((CDR_price[t])) for t in T)) +  #sum(DiscountFactor[t]*(rc_r[t].*(CDR_price[t])) for t in T)  +
    # Penalty term ETS market
    ( ρ2./2) * sum( DiscountFactor[t]*(((rc_r[t]) -(rc_r_bar[t] - 1/N*(rc_r[t] -  rc_bar[t])))^2) for t in T) .* a 
    )
...
function Update_CentralBank_recalibrate!(model::Model)
    ...
    model.ext[:objective] = @objective(model, Min, 
    # MACC
    (sum(rc_r[t].*((CDR_price[t])) for t in T)) + # sum(DiscountFactor[t]*(rc_r[t].*(CDR_price[t])) for t in T)  +
    # Penalty term ETS market
    # ( ρ1./2) * sum( DiscountFactor[t]*((rc_e[t] - (rc_e_bar[t] - 1/N*(CAP[t] + rc_e[t] -  ec_bar[t])))^2) for t in T) .* a   +
    # ( ρ1./2) * sum( DiscountFactor[t]*(((1/N*(CAP[t] + rc_e[t] -  ec_bar[t])))^2) for t in T) .* a   +
    ( ρ2./2) * sum( DiscountFactor[t]*(((rc_r[t]) -(rc_r_bar[t] - 1/N*(rc_r[t] -  rc_bar[t])))^2) for t in T) .* a 
...
function Joint_optimum!(REMOVALS::Bool)
    ...
    # Define variables
    e = model.ext[:variables][:e] = @variable(model, e[T], lower_bound=0, base_name="Emission allowances bought in year t by industrial fringe")
    r = model.ext[:variables][:r] = @variable(model, r[T], lower_bound=0, base_name="Emission allowances bought in year t by industrial fringe")
    ...
    model.ext[:objective] = @objective(model, Min, 
    # MACC
    sum((DiscountFactor[t])*(BETA*(EM_REF - e[t]).^(2)/2  + (A_r*r[t] + B_r*r[t].^(2)/(2*3000))) for t in T))
    ...
    EAC_price = [-dual.(model.ext[:constraints][:Cap_con][t]) for t in model.ext[:sets][:T]]
    ...
    plot!(twinx(p1), Timeseries_data[1:1:(T_OPT+1),"Year"], [[-dual.(model.ext[:constraints][:Cap_con][t])./DiscountFactor[t] for t in model.ext[:sets][:T]] [0/(1)^t  for t in model.ext[:sets][:T]]], ylims = (0, ymax), yticks = 50:50:450, label = ["MAC (EAC price)" "MRC"], color = ["grey" "grey83"], ylabel = [L"Certificate price [€/tCO$_2$]"], legend = (0.58,0.98), lw = 2, margin = 0mm)
    ...
    plot!(twiny(p3), Timeseries_data[1:1:(T_OPT+1),"Year"], xformatter=_->"", yformatter=_->"", xlabel = "", ylabel="", label = "", grid= false, gridlinewidth = 0, margin = 0mm)
    plot!(twinx(p3), Timeseries_data[1:1:(T_OPT+1),"Year"], xformatter=_->"", yformatter=_->"", xlabel = "", ylabel="", label = "",grid= false, gridlinewidth = 0, margin = 0mm)

    l = @layout [grid(1,2)]
    p4 = plot(p1,p3, size  = (1000, 400), layout =l, titleloc = :right, titlefont = font(11), xlim = [2024,(2024+T_OPT)], plot_title = L"\parbox{50em}{\centering 1.2: Net Emission Cap - Conditionally Connected (Threshold: 250 €/tCO$_2$) \\}", plot_titlefontsize = 12, topmargin = 2mm)

    return model
end


function plot_model!(Case_name::String, C_nr, model_e, model_r, model_ccb, ADMM, results, dict::Dict, plotting::Bool)
    ...
    if !plotting
        ...
        # Step 6: 
        RC_E_2 = results["rc_e"][ADMM["n_iter"],:]
        EM = results["e"][ADMM["n_iter"],:]
        REM = results2["r"][ADMM2["n_iter"],:] 
        ...
        Pcdr = results2["CDR_price"][ADMM2["n_iter"],:]
        ...
        CCB_surplus = [sum((results2["rc_r"][ADMM2["n_iter"],t+1] -results["rc_e"][ADMM["n_iter"],t+1])  for t in 0:i) for i in model_r.ext[:sets][:T]]
        RC_E = [rc_e[i] for i in model_ccbr.ext[:sets][:T]]
        # CCB_surplus = [sum((results["rc_r"][ADMM2["n_iter"],t+1] -results["rc_e"][ADMM["n_iter"],t+1])  for t in 0:i) for i in model_r.ext[:sets][:T]]


        Color =["snow3" "snow2"  "slategray3"]
        Alpha = [1 0.5 0.7]
        ymax = 650.0
        pp = stackedarea(Timeseries_data[1:1:(T_OPT+1),"Year"], [EM -REM],   ylims = (-200, 3000),  yticks =0:500:3000, labels = ["Emissions"  "Removals"], foreground_color_legend = nothing, title = "", ylabel = L"Emissions or Removals [MtCO$_2$/yr]", xlabel = "Years", legend = (0.02, 0.80), linecolor = Color, color = Color, alpha = Alpha, right_margin = 5mm)    
        ...
        # plot!(Timeseries_data[1:1:(T_OPT+1),"Year"], [RC_E], color =:green, labels="" ) 
        plot!(twiny(pp), Timeseries_data[1:1:(T_OPT+1),"Year"], [Timeseries_data[1:1:(T_OPT+1), "ETS_cap [EUA/y]"]./1 RC_E CCB_surplus], ylims = (-200, 3000), label = ["ETS cap" "Released RC to ETS" "RC in reserve"], foreground_color_legend = nothing, xformatter=_->"", yformatter=_->"", xlabel = "", ylabel="", legend = (0.02,0.98), lw =[2 1.5 1.5], color = ["black"  "black" "black" "brown"], ls = [:solid :dot :dash :dash], margin = 0mm)
        plot!(twinx(pp), Timeseries_data[1:1:(T_OPT+1),"Year"], [[Peac[t]   for t in 1:1:(T_OPT+1)] [Pcdr[t]  for t in 1:1:(T_OPT+1)]], ylims = (0, ymax), yticks = 50:50:ymax, label = ["ETS price (MAC)" "RC price (MRC)"], foreground_color_legend = nothing, color = ["grey36" "grey83"], ylabel = [L"Certificate price [€/tCO$_2$]"], legend = (0.58,0.98), lw = 2, margin = 0mm)
        plot!([2040, 2040], [0, 3000], lw =1, linecolor = :black, color= :white,  label="", ls = :dot)
        ...
        annotate!(2031.5, 2250,text("/////", :white, :right, 6), linewidth =2)
        annotate!(2032, 2455,text("___", :grey, :right, 8), linewidth =2)
        annotate!(2032, 2070,text("___", :grey, :right, 8), linewidth =2)
        ...
        plot!(twiny(ppp), Timeseries_data[1:1:(T_OPT+1),"Year"], [(EM-REM) CCB_surplus], lw=[0 1], xticks = (2025:5:(2024+T_OPT)), ylims = (-600, 2500), label = ["Net emissions" "RC in reserve"],foreground_color_legend = nothing, xformatter=_->"", yformatter=_->"", xlabel = "", ylabel="", legend = (0.02,0.91), color = ["black"  "black" "black" "brown"], ls = [:dash :dash :dash :dash], margin = 0mm, markershape = [:xcross :xcross], markersize = [2 1])
