function ADMM_price_based_m!(Case_name::String, results::Dict,ADMM::Dict, model_e::Model, model_r::Model, model_ccb::Model, dict::Dict, ETS_data::Dict, LINEAR::Bool) #, conditional_case::Bool)
    ...
    Total_budget = sum((results["ec"][ADMM["n_iter"],:].*[values(EAC_price[t]) for t in 0:1:T_OPT] + results["x"][ADMM["n_iter"],:].*[values(CDR_price[t]) for t in 0:1:T_OPT]  -results["rc"][ADMM["n_iter"],:].*[values(CDR_price[t]) for t in 0:1:T_OPT])) 
    ...
    credit_costs_emitter_avg = sum([EAC_price[t].*results["ec"][ADMM["n_iter"],t+1] + CDR_price[t].*results["x"][ADMM["n_iter"],t+1]  for t in 0:1:T_OPT])./sum([results["e"][ADMM["n_iter"],t+1] for t in 0:1:T_OPT]) # NOTE: ec also includes rc 
...
function Emitter!(model::Model)
    ...
    FRAC_RO = model.ext[:parameters][:FRAC_RO] 
    ...
    (sum(x[i] for  i in 0:t)  >= sum(FRAC_RO[i].*e[i]   for i in 0:t)))
...
function CentralBank!(model::Model)
    ...
    (sum(rc_e[i] + x_e[i] for  i in 0:t)  <= sum(rc_r[i]   for i in 0:t)))

    model.ext[:objective] = @objective(model, Min, 
    # MACC
    (- sum(DiscountFactor[t]*rc_e[t].*((EAC_price[t])) for t in T)) + sum(DiscountFactor[t]*((rc_r[t]-x_e[t]).*(CDR_price[t])) for t in T)  +
    # Penalty term ETS market
