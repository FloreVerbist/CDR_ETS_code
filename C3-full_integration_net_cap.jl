function ADMM_price_based_m!(Case_name::String, results::Dict,ADMM::Dict, model_e::Model, model_r::Model, model_ccb::Model, dict::Dict, ETS_data::Dict, LINEAR::Bool) #, conditional_case::Bool)
    ...
    for iter in iterations
        if convergence == 0
            ...
            for t in  model_e.ext[:sets][:T]
                ...
                _, j = findmin(abs.(RC(remov,t, Cum_remov).-LAMDA_CDR)) #FV_add: t+1
    ...
    push!(dict[(Case_name,"Learning_gains_per_rem")], (Total_costs_rem_NL -Total_costs_rem)/(Total_removals./FRAC_R)) # per gross removal 
...
# "Deflation Effect - Unconditional Integration (net cap)"
# plot!(twinx(pprint), Timeseries_data[1:1:(T_OPT+1),"Year"], [[Peac[t]   for t in 1:1:(T_OPT+1)] [Pcdr[t]  for t in 1:1:(T_OPT+1)]], ylims = (0, ymax), yticks = 50:50:ymax, xlim = [2024,(2024+T_OPT)], label = false,  foreground_color_legend = nothing, color = ["grey36" "grey83"], ls = [:dash :dash], lw = 2, margin = 0mm)
# groupedbar!(Timeseries_data[1:1:(T_OPT+1),"Year"], [EM -REM], bar_position =:stack, xticks = (2025:5:(2024+T_OPT)), xlim = [2024,(2024+T_OPT)], labels = false, foreground_color_legend = nothing,   ls = [:dash :dash], lw = 0.75, color = Color, linecolor = [:gray31 :gray31], alpha = Alpha, right_margin = 5mm)

function Joint_optimum_C2!()
    ...
    plot!(twinx(p1), Timeseries_data[1:1:(T_OPT+1),"Year"], [[EAC_price[t+1] for t in model.ext[:sets][:T]] [0/(1)^t  for t in model.ext[:sets][:T]]], ylims = (0, ymax), yticks = 50:50:ymax, label = ["MAC (EAC price)" "MRC"], color = ["grey36" "grey83"], ylabel = [L"Certificate price [€/tCO$_2$]"], legend = (0.58,0.98), lw = 2, margin = 0mm)
