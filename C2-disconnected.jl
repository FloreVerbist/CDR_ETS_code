function ADMM_price_based_m!(Case_name::String, results::Dict,ADMM::Dict, model_e::Model, model_r::Model, model_ccb::Model, dict::Dict, ETS_data::Dict, LINEAR::Bool) #, conditional_case::Bool)
    ...
    Total_budget = sum((results["ec"][ADMM["n_iter"],:].*[values(EAC_price[t]) for t in 0:1:T_OPT] -results["rc"][ADMM["n_iter"],:].*[values(CDR_price[t]) for t in 0:1:T_OPT])) # NOTE: we assume here that the CCB buys those removal credits themself and don't sell them to someone else
...
function Emitter!(model::Model)
    ...
    CAPr = model.ext[:parameters][:CAPr] # removal targets
...
function Update_emitter!(model::Model)
    ...
    ρ = model.ext[:parameters][:ρ] # Parameter controlling the price update step size in the ADMM algorithm
     CAP = model.ext[:parameters][:CAP] =   Dict(t =>  maximum([Timeseries_data[convert.(Int64,t+1), "ETS_cap [EUA/y]"], CAP_MIN]) for t in T) # cap  # cap  # cap
    CAPr = model.ext[:parameters][:CAPr] #removal targets 
    N = model.ext[:parameters][:N] # number of participants in the ETS market 
    Budget = model.ext[:parameters][:Budget]
...
function CentralBank!(model::Model)
    ...
    (sum(CAPr[i] for  i in 0:t)  <= sum(rc_r[i]   for i in 0:t)))

    model.ext[:objective] = @objective(model, Min, 
    # MACC
    sum(DiscountFactor[t]*(rc_r[t].*(CDR_price[t])) for t in T)  +
    # Penalty term ETS market
...
function plot_model!(Case_name::String, C_nr, model_e, model_r, model_ccb, ADMM, results, Dict_results::Dict, plotting::Bool)
    ...
    if !plotting
        ...
        CCB_surplus = [sum((rc_r[i] -rc_e[i] - x[i])  for i in 0:t) for t in model_r.ext[:sets][:T]]
        ...
        TARGETr = [Timeseries_data[t+1, "RC_cap [EUA/y]"]*1.0 for t in 0:T_OPT]
        # EA market plot

      


        #EM_fringe = results["e_f"][ADMM["n_iter"],:]
        ...
        plot!(twiny(pp), Timeseries_data[1:1:(T_OPT+1),"Year"], [CAPe TARGETr], ylims = (-200,3000), label = ["ETS cap" "RC target"], xformatter=_->"", yformatter=_->"", xlabel = "", ylabel="", legend = (0.02,0.98), color = ["black"  "black" "black" "brown"], foreground_color_legend = nothing, ls = [:solid :dash :dash :dash], margin = 0mm, lw=2)
        ...
        plot!(twiny(ppp), Timeseries_data[1:1:(T_OPT+1),"Year"], [CAPe -TARGETr], lw=[1 1], xticks = (2025:5:(2024+T_OPT)), ylims = (-600, 2500), label = ["ETS cap" "Removal target"],foreground_color_legend = nothing, xformatter=_->"", yformatter=_->"", xlabel = "", ylabel="", legend = (0.02,0.98), color = ["black"  "black" "black" "brown"], ls = [:solid :dash :dash :dash], margin = 0mm)
        plot!(twiny(ppp), Timeseries_data[1:1:(T_OPT+1),"Year"], [EM-REM], lw=0, xticks = (2025:5:(2024+T_OPT)), ylims = (-600, 2500), label = ["Net emissions"],foreground_color_legend = nothing, xformatter=_->"", yformatter=_->"", xlabel = "", ylabel="", legend = (0.02,0.86), color = ["black"  "black" "black" "brown"], ls = [:dash :dash :dash :dash], margin = 0mm, markershape = :xcross, markersize = 2)
