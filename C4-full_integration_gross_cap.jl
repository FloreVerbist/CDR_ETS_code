function ADMM_price_based_m!(Case_name::String, results::Dict,ADMM::Dict, model_e::Model, model_r::Model, model_ccb::Model, dict::Dict, ETS_data::Dict, LINEAR::Bool) #, conditional_case::Bool)
    ...
    for t in 0:T_OPT
        if value.(model_r.ext[:parameters][:r][t]) == 0.0 
            ...
            results["CDR_price"][ADMM["n_iter"],t+1] = RC(0.0, t,  Cum_remov_extract)[1]
        else
        end
    end
...
function Emitter!(model::Model)
    ...
    CAP = model.ext[:parameters][:CAP] =  Dict(t =>  maximum([Timeseries_data[convert.(Int64,t+1), "ETS_cap [EUA/y]"], CAP_MIN]) for t in T)
    ...
    # he doesn't know which fraction are removal credits and which not. 
...
function CentralBank!(model::Model)
    ...
    # Penalty term ETS market
    ( ρ1./2) * sum( DiscountFactor[t]*(((1/N*(CAP[t]  -  ec_bar[t])))^2) for t in T) .* a   +
...
function plot_model!(Case_name::String, C_nr, model_e, model_r, model_ccb, ADMM, results, Dict_results::Dict, plotting::Bool)
    ...
    if !plotting
        ...
        plot!(twiny(pp), Timeseries_data[1:1:(T_OPT+1),"Year"], [CAPe], ylims = (-200,3000), label = ["ETS cap"], xformatter=_->"", yformatter=_->"", xlabel = "", ylabel="", legend = (0.02,0.98), color = ["black"  "navy" "black" "brown"], foreground_color_legend = nothing, ls = [:solid :dash :dash :dash], margin = 0mm, lw=2)
